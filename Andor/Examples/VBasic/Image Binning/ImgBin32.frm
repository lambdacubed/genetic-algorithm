...
Begin VB.Form Form1 
   BackColor       =   &H00808000&
   Caption         =   "AndorSDK Visual Basic Example Drivers:- Image Binning"
   ClientHeight    =   8790
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8310
...
'acquire a Single Scan, Sub-Image with binning.
'
...
'source). Binning groups blocks of pixels into a super
'pixel whose value equals the sum of its constituent pixel values.
'Binning can be set for each dimension independently.
'
...
Function vbSetImage()
'****************************************************************
'Set the format of the image to be retreived ie. subimage dimensions
'and binning size
'****************************************************************
  vbSetImage = Not LOCALERROR
  
  Call vbNormaliseImage
  ...
  HBin = ebHBin
  VBin = ebVBin
  HSize = (HEnd - HStart + 1) / HBin
  VSize = (VEnd - VStart + 1) / VBin
  
  test = SetImage(HBin, VBin, HStart, HEnd, VStart, VEnd)
  If (test <> DRV_SUCCESS) Then
    vbSetImage = LOCALERROR
    buf = "Set Image ERROR: Can't set image."
  Else
    buf = newLine & "**SubImage = "
    buf = buf & Str(HStart) & "," & Str(HEnd) & ","
    buf = buf & Str(VStart) & "," & Str(VEnd) & newLine
    buf = buf & "**Binning = " & Str(HBin) & "x" & Str(VBin)
    buf = buf & newLine
  End If
  ebStatusBox.text = buf
End Function
...
'****************************************************************
'Get actual Exposure time used by AndorSDK.  AndorSDK
...
'**************************************************************
' Configure AndorSDK to operate in Single Scan mode.
'**************************************************************
...
'**************************************************************
' Configure AndorSDK to Full Resolution Image.
'**************************************************************
...
'************************************************************************
'Tells AndorSDK the type of trigger pulse to expect.  Internal means
...
Function vbStartAcq()
'**************************************************************
' Tells AndorSDK to begin Acquisition upon receipt of a trigger
...
'**************************************************************
  ...
  ReDim aData(1 To HSize, 1 To VSize)
  test = GetAcquiredData(aData(1, 1), HSize * VSize)
...
'*************************************************************************************
'This routine is the heart of the Image application.  It configures the AndorSDK
...
'triggering then YOU must supply the trigger pulse to the AndorSDK system.
...
Sub vbNormaliseImage()
'***************************************************************
'Subroutine normalises the sub image bounds to a state where the
'sub image dimensions are divisible by the corresponding binning
'factor which is a requirement of the AndorSDK SetImage() Function
'***************************************************************
  Dim hs As Long, he As Long, vs As Long, ve As Long
  Dim HBin As Long, VBin As Long
  ...
  he = hs + (Round((he - hs + 1) / HBin - 0.5) * HBin) - 1
  ve = vs + (Round((ve - vs + 1) / VBin - 0.5) * VBin) - 1
